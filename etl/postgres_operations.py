...
class PostgresEnricherPersonsOnly(PostgresMixin):
    """collect all films by person"""
    path: str = cache_conf.enricher_persons

    def __init__(self,
                 ready_producer: PostgresProducer,
                 modified_after: datetime,
                 ) -> None:
        # TODO found that modified_after is unused here. need to refactor Mixin
        super().__init__(modified_after)
        self.ready_producer = ready_producer
        self.postgres_saver = ready_producer.postgres_saver
        self.all_persons_uuid = list(map(lambda x: x['id'],
    ...
    @write_operations_state()
    def get_unlimited_person_links(self) -> list:
        """get_person_links without limits, modified_after not used"""
        if not self.all_persons_uuid:
            return []
        all_uuid_str = ','.join(map(lambda x: f"'{x}'",
                                    self.all_persons_uuid))
    ...
    def _collect_methods(self) -> tuple:
        return self.get_unlimited_person_links,
...
class PostgresMerger(PostgresMixin):
    ...
    @write_operations_state()
    def get_persons_linked(self) -> list:
        films_via_person = list(map(lambda x: x['id'], self.person_results))
        films_uuid = set(films_via_person)
        if not films_uuid:
            return []
        all_uuid_str = ','.join(map(lambda x: f"'{x}'", films_uuid))
        query = f"""
        ...
        # added `is not null` check. where is 2 m2m link - film without person.
        result = self.postgres_saver.execute(query)
        return result

    def _collect_methods(self) -> tuple:
        return self.get_films_linked, self.get_persons_linked
